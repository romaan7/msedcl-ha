...
Typed models for the MSEDCL responses, mapped to the ACTUAL JSON field names
observed from live responses (not the decompiled DTO guesses). Pair with
`maha_client.py`:

    from maha_client import MahaClient
    from parse import ConsumerInfo, CurrentReading, DailyPoint, MonthlyPoint, BillHistoryRow

    c = MahaClient.from_env()
    info = ConsumerInfo.from_contact_details(c.contact_details())
    print(info.name, info.bu, info.amisp_code, info.meter_number)

    cr = CurrentReading.parse(c.current_reading())
    print(cr.reading, cr.mtd_consumption, cr.reading_dt)

    days = DailyPoint.parse_list(c.daily_consumption("202607"))

Design notes
------------
* The API returns many numbers as strings ("3103.567", "0") and mixes cases
  (READING vs reading). Parsers coerce defensively and never raise on a missing
  optional field — unknown/missing values become None.
* Net metering: consumption endpoints report import/export/net. For a solar
  consumer, NET is often negative (you export more than you import).
"""
...
@dataclass
class DailyPoint:
    date_str: Optional[str] = None          # YYYYMMDD
...
if __name__ == "__main__":
    contact = {
        "MobileNo": "0000000000", "EmailId": "x@example.com", "Uid": "000000000000",
        "Consumer": {
